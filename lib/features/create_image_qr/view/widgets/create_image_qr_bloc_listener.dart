...
class CreateImageQrBlocListener extends StatelessWidget {
  const CreateImageQrBlocListener({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocListener<CreateImageQrCubit, CreateImageQrState>(
      listener: (context, state) {
        if (state.status == Status.loading) {
          ...
          GoRouter.of(context).pushNamed(GeneratedQrView.generatedQrView,
              extra: context.read<CreateImageQrCubit>().imageUrl);
